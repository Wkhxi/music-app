...
import { changeShowPlayList, changeCurrentIndex, changePlayMode, changePlayList, deleteSong,changeSequencePlayList, changeCurrentSong, changePlayingState } from "../store/actionCreators";
...
function PlayList(props) {
  ...
  const handleTouchEnd = (e) => {
    ...
    // 阈值为150px
    if(distance >= 150) {
      // 拖动列表 下滑的距离 大于150px 直接关闭PlayList
...
const mapStateToProps = (state) => ({
  ...
  playList: state.player.getIn(['playList']),//播放列表
  sequencePlayList: state.player.getIn(['sequencePlayList']),//顺序排列时的播放列表
